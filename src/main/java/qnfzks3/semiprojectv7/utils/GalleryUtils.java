package qnfzks3.semiprojectv7.utils;
...
public class GalleryUtils {
    ...
    public void makeThumbnail(GalAttach ga, Object uuid) {
        ...
        // 파일 이름을 ;으로 나눈 뒤 첫 번째 파일 명을 추출
        String basename = ga.getFname().split(";")[0];
        
        
        //서버에 업로드된 파일로 제정의 : 썸내일 생성 시 참고할 파일
        String refname = saveImgDir + basename;  

        //썸네일 이미지 경로 정의
        String thumbname = saveImgDir + "_thumbs/small_" + basename;  //원래 파일에 경로 잡아줌
        ...
        try {
            ...
            //지정한 위치를 기준으로 잘라냄
            //crop(대상, x좌표 , y좌표 , 잘라낼너비, 잘라낼 높이 ,투명도) - 이미지가 있을 때 이미지를 잘라내는 방법은
            //시작 지점을 기준으로 잘라내거나 , 시작점에서 얼만큼 떨어진 위치로부터 잘라냄
